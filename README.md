...
* 本地环境：
  ...
  6. **提交完成情况**。当做完部分或所有练习之后，在rustlings目录下执行 `git add .; git commit -m "update"; git push` 命令，把更新推送到自己的训练营仓库，由GitHub Actions自动评测。你可以在github仓库页面的actions分页看到你的CI提交结果，或者训练营官网查看自己的评分。
* 在线环境：
